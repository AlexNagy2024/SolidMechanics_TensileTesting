...
NE = 1000; % range to calculate the elasticity, this is picked off the previous plot
NY = 5775; % range to plot, this is picked off the previous plot
figure, hold on;
...
Eline = [0,0;EStress(NE)/E,EStress(NE)+5000]; % define E line with x,y locations at each end
...
title('Group 10 Aluminum Engineering Strain vs. Engineering Stress');
xlim([-0.01 0.16]); ylim([-0.2 5*10^4]);

% plot offset line and yield stress
offsetline = [0.002,0;EStress(NE)/E+.002,EStress(NE)+5000];
...
% fracture point
plot(EStrain(5763),EStress(5763),'b*')
...
Rsquared = 1-(SSE/SST);
text(0.0825,3.4*10^4,sprintf('R^{2} = %.5f',Rsquared))
